...
This is due to memory alignment: for efficiency reasons, the compiler pads the struct to make its size a multiple of 4.
...
There's no need for `#pragma pack` directives since the default memory layout has no padding.
See memory alignment slides to understand why.
