...
This is due to link:./memory-alignment.asciidoc[memory alignment]: for efficiency reasons, the compiler pads the struct to make its size a multiple of 4.
...
There's no need for `#pragma pack` directives since the link:./memory-alignment.asciidoc[default memory layout] has no padding.
